...
import type { Theme, TrustCapabilities } from "@repo/schemas";
...
type Tab = "general" | "providers" | "diagnostics";
...
const FOOTER_SHORTCUTS: Record<Tab, Shortcut[]> = {
  general: [
...
export function SettingsPage() {
  ...
  const [trustCapabilities, setTrustCapabilities] = useState<TrustCapabilities>({
    readFiles: true,
    readGit: true,
    runCommands: false,
  });
  ...
  const footerShortcuts = useMemo(
    () => FOOTER_SHORTCUTS[activeTab],
    [activeTab]
  );

  const rightShortcuts = useMemo(() => {
    if (activeTab === "providers" && selectedProvider) {
      return [
        { key: "PROV:", label: `${selectedProvider.name} - MOD: ${presetLabel}` },
      ];
    }
    return [];
  }, [activeTab, selectedProvider, presetLabel]);
  ...
  return (
    ...
    >
      <div className="border-b border-[--tui-border] px-4 py-2">
        <Tabs.List>
      ...
      <Tabs.Content value="general" className="flex-1 overflow-y-auto p-6">
        <div className="max-w-2xl mx-auto space-y-6">
          <Card title="THEME">
            <ThemeSelectorContent value={theme} onChange={setTheme} />
          </Card>

          <Card title="TRUST & PERMISSIONS">
            <TrustPermissionsContent
              directory="~/projects/stargazer"
              value={trustCapabilities}
              onChange={setTrustCapabilities}
              showActions
              isTrusted={
                trustCapabilities.readFiles || trustCapabilities.readGit
              }
              onSave={() => console.log("Save trust settings")}
              onRevoke={() =>
                setTrustCapabilities({
                  readFiles: false,
                  readGit: false,
                  runCommands: false,
                })
      ...
      <Tabs.Content value="providers" className="flex-1 overflow-hidden">
          ...
          <div className="w-2/3 flex flex-col bg-[#0f131a]">
            <div className="p-3 border-b border-[--tui-border] bg-[--tui-selection]/30 flex justify-between">
              ...
              {selectedProvider?.status === "configured" && (
                <span className="text-xs text-[--tui-green] font-mono">
                  Active
                </span>
              )}
            </div>
            ...
            <div className="p-8 flex-1 flex flex-col gap-8">
              <div>
                ...
                <p className="mt-3 text-xs text-gray-500 max-w-md">
                  ...
                  Uses{" "}
                  <span className="text-[--tui-fg]">{presetInfo.model}</span>.{" "}
              ...
              <div className="mt-auto mb-4">
                ...
                >
                  ...
                  <span className="border-b border-dashed border-gray-700 group-hover:border-[--tui-fg]">
                    {showAdvanced ? "Hide" : "Show"} Advanced Settings (Model
                    ID, Temp, Timeout)
                  </span>
      ...
      <Tabs.Content
        value="diagnostics"
        className="flex-1 overflow-hidden px-4 flex justify-center items-center"
      >
