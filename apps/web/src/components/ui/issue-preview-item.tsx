import { cn } from '../../lib/utils';
import type { SeverityLevel } from './severity-bar';

export interface IssuePreviewItemProps {
  title: string;
  file: string;
  line: number;
  category: string;
  severity: SeverityLevel;
  onClick?: () => void;
  className?: string;
}

const SEVERITY_ICONS: Record<SeverityLevel, { icon: string; color: string }> = {
  blocker: { icon: '✖', color: 'text-tui-red' },
  high: { icon: '▲', color: 'text-tui-yellow' },
  medium: { icon: '●', color: 'text-gray-400' },
  low: { icon: '○', color: 'text-tui-blue' },
};

const SEVERITY_BADGE_STYLES: Record<SeverityLevel, string> = {
  blocker: 'border-tui-red text-tui-red',
  high: 'border-tui-yellow text-tui-yellow',
  medium: 'border-gray-400 text-gray-400',
  low: 'border-tui-blue text-tui-blue',
};

const SEVERITY_LABELS: Record<SeverityLevel, string> = {
  blocker: 'BLOCKER',
  high: 'HIGH',
  medium: 'MEDIUM',
  low: 'LOW',
};

export function IssuePreviewItem({
  title,
  file,
  line,
  category,
  severity,
  onClick,
  className,
}: IssuePreviewItemProps) {
  const { icon, color } = SEVERITY_ICONS[severity];

  return (
    <button
      type="button"
      onClick={onClick}
      className={cn(
        'flex items-center justify-between p-3 w-full text-left',
        'bg-tui-bg border-b border-gray-800 last:border-b-0',
        'hover:bg-tui-selection group cursor-pointer transition-colors',
        className
      )}
    >
      <div className="flex items-center gap-4">
        <span className={cn('font-bold text-lg', color)} aria-hidden="true">
          {icon}
        </span>
        <div>
          <div className="text-sm font-bold group-hover:text-tui-blue transition-colors">
            {title}
          </div>
          <div className="text-xs text-gray-500 font-mono">
            {file}:{line}
          </div>
        </div>
      </div>
      <div className="flex items-center gap-4">
        <span className="hidden sm:inline text-xs text-gray-500">{category}</span>
        <span
          className={cn(
            'text-[10px] border px-1.5 py-0.5 uppercase tracking-wide',
            SEVERITY_BADGE_STYLES[severity]
          )}
        >
          {SEVERITY_LABELS[severity]}
        </span>
      </div>
    </button>
  );
}
