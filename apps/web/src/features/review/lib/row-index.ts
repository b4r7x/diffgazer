 ...
 */
function isRowEvent(event: ReviewEvent, source: string | undefined): boolean {
...
// The TUI runs the same continuation/eviction/append windowing contract over
// the same core primitives in cli/diffgazer/src/features/review/lib/visible-events.ts,
// projected to retained events instead of logical row indices. Change both together.
...
export function convertEventRowWindow(
  ...
  for (const logicalIndex of window) {
    ...
    if (!event) continue;
    entries.push(convertReviewEventToLogEntry(event, logicalIndex));
  }
  return entries;
}
