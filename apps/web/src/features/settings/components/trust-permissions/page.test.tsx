...
function createTestApi(init = makeInitResponse(null)): BoundApi {
  return {
...
describe("SettingsTrustPermissionsPage", () => {
  ...
  it("resets the draft when async trust data arrives", async () => {
    ...
    vi.mocked(testApi.loadConfigurationInit).mockResolvedValue(makeInitResponse(TRUSTED_FIXTURE));
    await refetchInit();
  ...
  it("does not steal focus from the action row when async trust data arrives", async () => {
    ...
    vi.mocked(testApi.loadConfigurationInit).mockResolvedValue(makeInitResponse(updatedFixture));
    await refetchInit();
  ...
  it("saves edited trust permissions, confirms success, and stays on the editor", async () => {
    const user = userEvent.setup();
    renderPage(makeInitResponse(TRUSTED_FIXTURE));
    await waitForConfigReady();
