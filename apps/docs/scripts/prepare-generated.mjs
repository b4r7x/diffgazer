...
const DOCS_LIBRARIES_CONFIG_PATH = resolve(
	DOCS_ROOT,
	"config/docs-libraries.json",
);
...
function shouldPrepareLibraryArtifacts() {
	...
	const missingFiles = collectMissingWorkspaceArtifactFiles(
		WORKSPACE_ROOT,
		artifactLibraries,
	);
...
if (shouldPrepareLibraryArtifacts()) {
	execFileSync(
		"pnpm",
		["--dir", WORKSPACE_ROOT, "run", "prepare:library-artifacts"],
		{
			stdio: "inherit",
		},
	);
...
await import(
	pathToFileURL(resolve(DOCS_ROOT, "scripts/sync-artifacts.mjs")).href
);
await import(
