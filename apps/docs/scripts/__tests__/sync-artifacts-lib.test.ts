...
// @ts-expect-error runtime ESM helper module is intentionally untyped
import { assertArtifactSyncOutputs, collectArtifactSyncValidationErrors, getArtifactLibraries, parseDocsLibrariesConfig, readDocsLibrariesConfig, resolveArtifactSyncMode } from "../sync-artifacts-lib.mjs"
...
describe("sync artifacts helper config parsing", () => {
  ...
  it("switches sync mode based on DIFFGAZER_DEV", () => {
    ...
    expect(resolveArtifactSyncMode({ DIFFGAZER_DEV: "1" } as NodeJS.ProcessEnv)).toBe("workspace")
  })
  ...
  it("auto-falls back to workspace when local artifact packages are not resolvable", () => {
    const mode = resolveArtifactSyncMode(
      {} as NodeJS.ProcessEnv,
      {
        ...
        resolvePackage: (packageName: string) => packageName !== "@b4r7x/diff-ui-artifacts",
      },
    )
...
describe("sync artifacts helper output validation", () => {
  ...
  it("reports missing namespace outputs and primary mismatch", () => {
    ...
    expect(errors).toContain("Generated namespace is empty: src/generated/diff-ui")
  })
})
