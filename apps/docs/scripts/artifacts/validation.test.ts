...
const { makeTempDir, cleanupTempDirs } = createTempDirs(
...
describe("artifact validation", () => {
	...
	it("fails closed when fingerprint inputs are missing", () => {
		...
		expect(
			validateLibraryArtifacts({ rootDir: root, label: "fixture" }),
		).toContain("fixture: missing fingerprint input docs/generated");
	});
	...
	it("rejects artifact manifest paths that escape the artifact root", () => {
		...
		expect(
			validateLibraryArtifacts({ rootDir: root, label: "fixture" }).join("\n"),
		).toContain(
	...
	it("fails tampered integrity bundles", () => {
		...
		).toMatch(/keys bundle: bundle integrity mismatch/);
	});
	...
	it("requires every declared integrity bundle key", () => {
		...
		).toContain("registry bundle: bundle styles is missing");
	});
				...
				files: [
					{
						...
						content:
							'import { restoreFocus } from "./utils/focus-restore.js";\n',
					},
				],
			},
		];
