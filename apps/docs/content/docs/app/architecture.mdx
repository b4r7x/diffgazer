...
```text
git diff -> context -> provider request -> enrich -> report
```

The server reads repository state, builds context, calls the configured model provider, enriches the response, and stores review history locally. The pipeline is described in more detail in [The review pipeline](/app/concepts/review-pipeline).
