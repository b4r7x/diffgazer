...
Local HTTP products: Diffgazer verifies only that the **first network hop is loopback**. Any downstream routing, data residency, storage, or telemetry beyond that hop is the server operator's responsibility. Ollama Cloud is not the Ollama loopback transport; it is a separate hosted product that sends review content to ollama.com.
...
- **Your key, only to its own endpoint.** For a saved Z.AI, Groq, Cerebras, Mistral, DeepSeek, Qwen International, or Moonshot Open Platform configuration that has a credential, Diffgazer asks that configuration's own endpoint for its model list (`{endpoint}/models`) using that configuration's own credential. A credential is sent only to the endpoint of the configuration it belongs to — never to models.dev, never to a public list, never to another product's endpoint. Google Gemini uses a different list API and stays catalog-only, so model discovery never sends a Gemini key; only a review or an explicit Verify does. Each list is cached for five minutes per configuration.
...
Local HTTP and local CLI products do not expose picker-side model discovery in the setup wizard today — their **Model** step shows **Catalog observations are unavailable for this configuration product.** with **Retry** only, and exact model IDs must be selected through the local API.
