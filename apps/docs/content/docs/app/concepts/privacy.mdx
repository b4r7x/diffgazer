...
- For Google Gemini, Z.AI, Groq, Cerebras, Mistral, and OpenRouter, Diffgazer may read the public models.dev catalog at `https://models.dev/api.json`. That request carries no key and no code. `DIFFGAZER_OFFLINE` skips the live fetch.
- DeepSeek, Qwen International, and Moonshot Open Platform discover models through their configured hosted tuple rather than the shared models.dev snapshot. Local HTTP and local CLI products do not expose picker-side model discovery in the setup wizard today — their **Model** step shows **Catalog observations are unavailable for this configuration product.** with **Retry** only, and exact model IDs must be selected through the local API.
