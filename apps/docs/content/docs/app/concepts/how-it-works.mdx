...
Your **product configuration** is the model transport you bring. Diffgazer supports fourteen selectable products documented in the [providers reference](/app/reference/providers). Web, Ink, and CLI all consume the same client-safe configuration summaries and readiness language.
...
Diffgazer does not fall back to another product when admission or transport fails. Catalog-backed hosted products use the models.dev catalog; OpenRouter and Ollama Cloud also read their public model lists, and a saved OpenAI-compatible configuration with a key asks its own endpoint for its list. For exactly what each request carries, see [Privacy](/app/concepts/privacy).
