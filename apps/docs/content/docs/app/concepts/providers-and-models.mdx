...
The picker hides only two kinds of model: models that cannot emit text, and — for strict JSON-schema products like Google Gemini — models the catalog marks as unable to return structured output. Nothing is hidden by billing pool: on a dual-pool product like OpenCode Zen the picker lists the union of both pools, each row names the pool that will bill it, and choosing a model moves the configuration to that pool along with it. A row only one pool serves names that pool; a row both pools serve names whichever pool is armed in the picker's own **Zen**/**Go** selector, which never changes the rows listed and only decides which wallet a shared model bills. OpenRouter is the exception to the second: every pinned route is listed — its gateway applies schema enforcement where the route supports it and Diffgazer validates the response locally either way. A model no source says anything about is listed; your first review verifies it.
