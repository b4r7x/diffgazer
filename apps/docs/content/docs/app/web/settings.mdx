...
Theme controls the look of the web UI. You choose between dark and light, with a live preview that follows whichever option is focused.

> Note: The stored theme schema also defines `auto` and `terminal` values — `terminal` is used by the TUI, and `auto` is a legacy web value that now resolves to dark. The schema is the source of truth for what can be saved; the web UI deliberately exposes only the two themes it renders.
