...
Pick a row to open that area. Configuration editors have Cancel and Save actions: they do not write changes before Save, and Cancel or `Esc` returns to the hub without applying them. The hub footer shows the project path this session is reviewing, plus a `local settings` caption. Your choices are persisted to `~/.diffgazer/config.json`, which the footer does not print. For the full shape of that file and every field it holds, see [configuration](/app/reference/configuration).
