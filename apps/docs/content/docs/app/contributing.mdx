...
Diffgazer is maintained but not in active development. If a fix comes in, it gets merged; a feature should start as an issue so the work is not wasted. See [Status](/app/status).
