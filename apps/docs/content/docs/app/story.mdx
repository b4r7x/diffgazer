...
Most AI review workflows make one of three tradeoffs.

They are **manual**, so you copy a diff, explain the repo, ask for findings, then rebuild that context for the next review.

They are **remote-first**, so source code, metadata, and review history move into a service before you have decided what should leave your machine.

They are **too shallow**, so the model only sees a raw patch and misses the surrounding code, project shape, or reason a change is risky.
...
- keep findings tied to files, context, severity, and rationale;
- avoid sending code anywhere except the provider you selected.

## Why local-first matters

Local-first is not just a privacy line. It also keeps the tool simple to reason about.
...
The next useful work is not to make diffgazer more magical. It is to make the current path steadier: clearer docs, safer release gates, better provider ergonomics, stronger registry handoff, and fewer places where a reviewer has to guess what the tool did.
