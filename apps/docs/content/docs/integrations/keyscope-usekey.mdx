...
The primary hook for binding keyboard shortcuts. Requires `KeyboardProvider` (silently does nothing without one). Not available in copy mode — install the full keyscope package.

> For the complete API reference including hotkey format and all options, see the [keyscope documentation](https://github.com/b4r7x/keyscope).
