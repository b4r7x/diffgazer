...
export const PRIMARY_DOCS_LIBRARY_ID =
	docsLibrariesConfig.primaryLibraryId as DocsLibraryId;
...
export function getDocsLibraryConfig(
	lib: DocsLibraryId,
): DocsLibraryConfigData {
...
function getLocalInstallCommand(
...
export function getLibrariesWithArtifacts(): (DocsLibraryConfigData & {
	...
	return getEnabledDocsLibraries().filter(
		(
			c,
		): c is DocsLibraryConfigData & { artifactSource: ArtifactSourceConfig } =>
			!!c.artifactSource,
	);
}

export function parseDocsLibrary(
...
export function getDocsLibraryFromPathname(
	pathname: string,
): DocsLibraryId | null {
...
export function getRouteSlugsFromPathname(
	pathname: string,
	library: DocsLibraryId,
): string[] {
...
const DOCS_PATH_PATTERN = new RegExp(
	`^/(?:${KNOWN_LIBRARY_IDS.join("|")})(?:/|$)`,
);
...
export function routeSlugsFromSourcePath(
	...
	const rawSlugs = normalizeRouteSlugs(
		sourcePath.slice(SOURCE_DOCS_PREFIX.length).split("/"),
	);
