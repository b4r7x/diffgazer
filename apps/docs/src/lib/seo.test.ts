...
describe("buildPageSeo", () => {
	it("emits a title, canonical link, and the required SEO meta tags", () => {
		...
		expect(findMeta(seo.meta, "name", "description")?.content).toBe(
			"Button primitive",
		);
		expect(findMeta(seo.meta, "property", "og:title")?.content).toBe(
			"Button - UI Docs",
		);
		...
		expect(findMeta(seo.meta, "property", "og:site_name")?.content).toBe(
			DEFAULT_SITE_NAME,
		);
		expect(findMeta(seo.meta, "name", "twitter:card")?.content).toBe("summary");
		expect(findMeta(seo.meta, "name", "twitter:title")?.content).toBe(
			"Button - UI Docs",
		);
...
describe("buildRootHeadDefaults", () => {
	...
	it("includes og:image meta tag", () => {
		...
		expect(findMeta(meta, "property", "og:image")?.content).toBe(
			`${PUBLIC_ORIGIN}/logo512.png`,
		);
	});
