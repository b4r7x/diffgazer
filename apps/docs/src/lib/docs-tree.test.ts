...
describe("findPageNeighbors", () => {
	const tree: PageTree = {
		name: "@diffgazer/ui",
		children: [
			{ type: "page", name: "Getting Started", url: "/ui/getting-started" },
			{
				type: "page",
				name: "Installation",
				url: "/ui/getting-started/installation",
			},
			{
				...
				url: "/ui/getting-started/consumption-modes",
			},
		],
	};

	it("returns the surrounding pages in sidebar order", () => {
		expect(findPageNeighbors(tree, "/ui/getting-started/installation")).toEqual(
			{
				previous: { name: "Getting Started", url: "/ui/getting-started" },
				next: {
					name: "Consumption Modes",
					url: "/ui/getting-started/consumption-modes",
				},
	...
	it("has no previous for the first page and no next for the last", () => {
		const first = findPageNeighbors(tree, "/ui/getting-started");
		expect(first.previous).toBeNull();
		expect(first.next?.url).toBe("/ui/getting-started/installation");

		const last = findPageNeighbors(
			tree,
			"/ui/getting-started/consumption-modes",
		);
		expect(last.next).toBeNull();
		expect(last.previous?.url).toBe("/ui/getting-started/installation");
	});
