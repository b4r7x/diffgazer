...
function collectHookDocExampleCounts(): Map<string, number> {
	...
	for (const file of listHookDocFiles()) {
		...
		counts.set(
			hook,
			examplesBlock
				? [...examplesBlock[1].matchAll(/name: "([a-z0-9-]+)"/g)].length
				: 0,
		);
	}
	return counts;
...
function collectMdxExampleRefs(): ExampleRef[] {
	...
	for (const file of listRepoFiles("libs/ui/docs/content", ".mdx")) {
		const source = readAbsolute(file);
		const item =
			frontmatterComponent(source) ??
			`${basename(file)} (no component frontmatter)`;
...
describe("docs-library source path mapping", () => {
	...
	it("maps source paths to route slugs only for the active library", () => {
		expect(
			routeSlugsFromSourcePath("ui", "/docs/ui/components/button"),
		).toEqual(["components", "button"]);
		expect(
			routeSlugsFromSourcePath("keys", "/docs/keys/guides/navigation"),
		).toEqual(["guides", "navigation"]);
		expect(
			routeSlugsFromSourcePath("ui", "/docs/keys/guides/navigation"),
		).toBeNull();
		expect(
			routeSlugsFromSourcePath("keys", "/docs/ui/components/button"),
		).toBeNull();
	});

	it("generates local install commands while packages are unpublished", () => {
		expect(getInstallCommand("ui", "button")).toBe(
			"pnpm exec dgadd add ui/button",
		);
		expect(getInstallCommand("ui", "ui/button")).toBe(
			"pnpm exec dgadd add ui/button",
		);
	...
	it("keeps installer handoff publish-gated and local-first", () => {
		expect(LOCAL_DGADD_PREREQUISITE).toContain(
			"locally packed @diffgazer/add tarball",
		);
	...
	it("marks provider-backed keys hooks as package-only while keeping package import metadata", () => {
		const meta = getConsumptionMetadata(
			"keys",
	...
	it("uses deterministic docs preview without npx network dependency", () => {
		...
		expect(docsPackage.scripts.preview).toBe(
	...
	it("documents the exact React peer floor for public docs handoff", () => {
		const docs = [
			...
			readRepoFile(
				"libs/ui/docs/content/getting-started/consumption-modes.mdx",
			),
	...
	it("loads synced keys demos through the generated demo loader", async () => {
		...
		render(
			createElement(Suspense, { fallback: "loading" }, createElement(Demo)),
		);
	...
	it("keeps public docs on current highlight and keyboard prop names", () => {
		...
		expect(docs).not.toMatch(
			/\bNavigationList\b[\s\S]{0,160}\bisHighlighted\b/,
		);
	...
	it("keeps public composition docs from promising opaque wrapper depth", () => {
		const docs = [
			readRepoFile("libs/ui/docs/content/patterns/compound-components.mdx"),
			...listRepoFiles("libs/ui/registry/component-docs", ".ts").map(
				readAbsolute,
			),
		].join("\n");

		expect(docs).not.toMatch(
			/no matter how deep|arbitrarily nested|deeply nested/i,
		);
	...
	it("every public UI component/hook page has consumption metadata block", () => {
		const componentPages = listRepoFiles(
			"libs/ui/docs/content/components",
			".mdx",
		);
	...
	it("documents every exported keys hook under hook handoff pages", () => {
		...
		const exportedHookSlugs = [
			...
			.sort();
		const documentedHookSlugs = listRepoFiles(
			"libs/keys/docs/content/hooks",
			".mdx",
		)
			.map((file) =>
			...
			.filter(
				(slug): slug is string => typeof slug === "string" && slug !== "index",
			)
			.sort();
	...
	it("docs-libraries.json does not point enabled libraries to missing content", () => {
		const config = JSON.parse(
			readRepoFile("apps/docs/config/docs-libraries.json"),
		);
		const enabledLibraries = config.libraries.filter(
		...
		for (const lib of enabledLibraries) {
			...
			expect(
				hasContent,
				`enabled library "${lib.id}" has no content in ${contentDir}`,
			).toBe(true);
		}
	});
