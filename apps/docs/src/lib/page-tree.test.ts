...
describe("findPageNeighbors", () => {
  ...
  it("returns the surrounding pages in sidebar order", () => {
    expect(findPageNeighbors(tree, "@diffgazer/ui/getting-started/installation")).toEqual({
      previous: {
        name: "Getting Started",
        url: "@diffgazer/ui/getting-started",
      },
  ...
  it("has no previous for the first page and no next for the last", () => {
    ...
    const last = findPageNeighbors(tree, "@diffgazer/ui/getting-started/consumption-modes");
    expect(last.next).toBeNull();
    expect(last.previous?.url).toBe("@diffgazer/ui/getting-started/installation");
  });
