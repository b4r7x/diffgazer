...
describe("isPrimaryNavigationClick", () => {
  ...
  it("rejects middle/right clicks and modifier clicks (open-in-new-tab gestures)", () => {
    ...
    expect(isPrimaryNavigationClick(clickEvent({ shiftKey: true }))).toBe(false);
...
describe("groupBySection", () => {
  it("titles the first section from its leading separator", () => {
    ...
    expect(sections.map((s) => s.title)).toEqual(["Getting Started", "Components"]);
    expect(sections[0]?.items.map((i) => i.url)).toEqual(["@diffgazer/ui/getting-started"]);
  });
...
describe("sidebarItemLabel", () => {
  function labelFor(sectionTitle: string, items: PageTreeNode[]) {
    ...
    return (item: PageTreeNode) => sidebarItemLabel(sectionTitle, indexUrl, item);
  }
  ...
  it("matches the section header case-insensitively", () => {
    const items = [page("Web mode", "/app/web"), page("Onboarding", "/app/web/onboarding")];
  ...
  it("collapses an index title that redundantly prefixes the section header", () => {
    const items = [page("API Overview", "/keys/api"), page("Types", "/keys/api/types")];
