...
): { section: string; itemId: string } | null {
...
function Captions({ items }: { items: Caption[] }) {
	const visible = items.filter((item): item is Required<Caption> =>
		Boolean(item.value),
	);
...
function PathBody({
	...
	return (
		<div className="space-y-4">
			...
			{path.note && (
				<Typography as="p" size="xs" className="leading-relaxed">
					{path.note}
				</Typography>
			)}
		</div>
...
export function ConsumptionBlock() {
	...
	const localDestination = meta.paths.copy.available
		? meta.copyPath
		: undefined;
	...
	const tabs: {
		...
		},
		{
			...
			captions: [{ label: "Import", value: meta.packageImport }],
		},
	];
		...
		<div className="space-y-5">
			...
			{meta.cssNote && library === "ui" && (
				<Typography
					as="p"
					size="xs"
					className="leading-relaxed border-t border-border/40 pt-3"
				>
					{meta.cssNote}
				</Typography>
			)}
