...
const files = [
	{
		...
		highlighted: [
			{ number: 1, content: [{ text: "export function Button() {}" }] },
		],
	},
];
...
describe("SourceViewer", () => {
	it("does not synthesize a public installer command when no installer is configured", () => {
		...
		expect(
			screen.getByRole("button", { name: /View component source/i }),
		).toBeInTheDocument();
	});
	...
	it("shows the configured install command", () => {
		...
		expect(
			screen.getByText("pnpm exec dgadd add ui/button"),
		).toBeInTheDocument();
	});
	...
	it("renders consumer-provided integration guidance alongside the install command", () => {
		render(
			<SourceViewer
				...
				installCommand="pnpm exec dgadd add ui/button"
				integrationNote={
					<span>Use --integration keys for the full experience.</span>
				}
			/>,
		);
	...
	it("shows the merged-source copy button only when merged source is provided", () => {
		...
		rerender(
			<SourceViewer files={files} mergedSource="export function Button() {}" />,
		);
