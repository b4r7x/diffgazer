...
import type { HomeLibrary } from "../home-data";
...
const LIBRARIES: HomeLibrary[] = [
	{
		...
		sections: [
			{ name: "Getting Started", splat: "getting-started/installation", count: 5 },
	...
	},
	{
		...
		sections: [
			{ name: "Getting Started", splat: "getting-started/installation", count: 3 },
...
describe("HomeView", () => {
	...
	it("links the three documentation areas to their roots", () => {
		...
		expect(
			within(areas).getByRole("link", { name: /^diffgazer AI code review/i }),
		).toHaveAttribute("href", "/app");

		expect(
			within(areas).getByRole("link", { name: /^@diffgazer\/ui/i }),
		).toHaveAttribute("href", "/ui");

		expect(
			within(areas).getByRole("link", { name: /^@diffgazer\/keys/i }),
	...
	it("surfaces the real component and hook counts on the area cards", () => {
		...
		const areas = screen.getByRole("region", { name: "Documentation areas" });
		expect(
			within(areas).getByText(/browse 47 components/i),
		).toBeInTheDocument();
	...
	it("titles the browse section with an accurate, non-recency heading", () => {
		...
		expect(
			screen.getByRole("heading", { level: 2, name: "Browse the docs" }),
		).toBeInTheDocument();
	...
	it("exposes real external links in the footer", () => {
		...
		const footerNav = screen.getByRole("navigation", { name: "External links" });
		expect(within(footerNav).getByRole("link", { name: "GitHub" })).toHaveAttribute(
			"href",
			"https://github.com/b4r7x/diffgazer",
		);
		expect(within(footerNav).getByRole("link", { name: "NPM" })).toHaveAttribute(
			"href",
			"https://www.npmjs.com/package/@diffgazer/ui",
		);
		expect(within(footerNav).getByRole("link", { name: "Docs" })).toHaveAttribute(
			"href",
			"/ui",
		);
	});
