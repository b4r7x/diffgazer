...
const resolveLibrarySwitchPath = createServerFn({ method: "GET" })
	...
	.handler(async ({ data }) => {
		...
		const targetSlugs = routeSlugsFromSourcePath(
			data.targetLibrary,
			targetPage.path,
		);
...
export function Header({ library }: HeaderProps) {
	...
	return (
		<header
			className="shrink-0 bg-background z-20 flex border-b border-border"
			aria-busy={isHeaderBusy}
		>
				...
				<div className="flex items-center gap-5">
					<Select value={library} onChange={(v) => void handleLibraryChange(v)}>
						...
						>
							...
							{isHeaderBusy && (
								<Spinner size="sm" className="ml-2 text-muted-foreground" />
							)}
						</SelectTrigger>
					...
					>
						...
						<Kbd size="sm" className="text-muted-foreground/50">
							/
						</Kbd>
					</button>
				</div>
