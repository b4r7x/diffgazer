...
import type { PageTreeNode } from "@/lib/docs-tree";
import {
	findSectionIndexUrl,
	groupBySection,
...
describe("groupBySection", () => {
	it("titles the first section from its leading separator", () => {
		...
		expect(sections[0]?.items.map((i) => i.url)).toEqual(["/ui/getting-started"]);
	});
...
describe("findSectionIndexUrl", () => {
	it("returns the item whose url is the parent of its siblings", () => {
		const items = [
			page("Hooks", "/ui/hooks"),
			page("Active Heading", "/ui/hooks/active-heading"),
		];
		expect(findSectionIndexUrl(items)).toBe("/ui/hooks");
	});

	it("returns null when no item is a parent of another", () => {
		const items = [
			page("Button", "/ui/components/button"),
			page("Card", "/ui/components/card"),
		];
