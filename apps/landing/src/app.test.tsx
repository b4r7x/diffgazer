...
describe("Landing App", () => {
  // The app runs from a secure context (https/localhost); jsdom leaves
  // isSecureContext undefined, so the clipboard guard would otherwise short
  // out the real copy path. Individual tests override this to exercise the
  // unavailable-clipboard branch.
  beforeEach(() => {
  ...
  it("renders the capitalized product wordmark as the page heading", () => {
    render(<App />);
    expect(screen.getByRole("heading", { level: 1, name: "Diffgazer" })).toBeInTheDocument();
  });
