...
Published packages use semantic versioning through changesets. The current package version is the `version` field in each package's `package.json`; do not duplicate current versions in docs. The one sanctioned exception is the `diffgazer.json` example in `cli/add/README.md`: it shows what `dgadd init` writes, and init stamps the package version into that file. `version-packages` rewrites the example (`scripts/monorepo/sync-add-readme-version.mjs`) so the Version PR carries the bump, and `cli/add/src/commands/init/readme.test.ts` pins it after merge.
...
Public packages are published through the root `pnpm run release` script. Its targeted publisher reads the npm versions of every public workspace package, preflights the set against the first-publish gate, and publishes each manifest version npm lacks with a separate filtered `pnpm publish` invocation. The registry, not the checked-out commit, decides the set: a Version PR whose own CI failed for a reason a later commit fixed still publishes from that later commit, and a run after a complete release is a no-op. Authentication and provenance both come from the workflow's `id-token: write` GitHub OIDC identity: each `pnpm publish` exchanges it for a short-lived token against the package's npm trusted publisher (GitHub Actions, owner `b4r7x`, repository `diffgazer`, workflow filename `release.yml`, no environment), and the explicit `--provenance` flag the publisher passes makes the attestation mandatory instead of a visibility-dependent default. pnpm 11's native publish reads neither `NPM_CONFIG_PROVENANCE` nor `publishConfig.provenance` (both are npm's switches), and it only attests on its own when npm's OIDC token exchange succeeds, which needs a trusted publisher, not a token — `@diffgazer/add@0.2.0` shipped without an attestation that way, and an attestation cannot be added to a published version afterwards. `publishConfig.provenance` stays on each package so a one-off `npm publish` applies the same policy. Scoped public packages set `publishConfig.access` to `public`. The `author` field is uniform across the four published packages (`"author": "diffgazer"`); the `license` field intentionally splits MIT vs Apache-2.0 per the [Licensing](#licensing) section.
...
3. No npm token is involved. pnpm 11 always attempts npm's OIDC token exchange first when it runs on GitHub Actions with `id-token: write`; a package whose exchange fails would fall back to a static `_authToken`, which this workflow does not configure, so it fails with an authentication error instead of publishing without provenance. Each of the four packages carries the trusted publisher named under [Publish Metadata](#publish-metadata), and the release guard (`scripts/monorepo/check-release-workflow-guards/release.mjs`) rejects a workflow that passes an npm token to any step or a `registry-url` input to any action.

**What changed and why (2026-09).** The workflow used to authenticate with a long-lived granular `NPM_TOKEN` secret while the OIDC identity only signed provenance. That token was revoked, the repository secret deleted, and a trusted publisher configured on every package, so the repository holds no npm credential that could be exfiltrated: a publish must originate from `release.yml` on `b4r7x/diffgazer`. A last-resort token path remains under [Recovery from publish failure](#recovery-from-publish-failure).

#### First-publish gate

An unfiltered workspace publisher would publish every public package whose version is absent from npm, including unrelated gated packages. To keep each npm gate mechanical, `pnpm run release` delegates publication to `scripts/monorepo/guard-publish.mjs`. The default invocation considers every non-private workspace package, reads its registry versions, and rejects the whole run before `pnpm` starts if any never-published package is absent from `FIRST_PUBLISH_ALLOWLIST`. The allowlist names all four release-managed packages (`diffgazer`, `@diffgazer/add`, `@diffgazer/ui`, `@diffgazer/keys`), so the gate protects against an unintended name only: a new public workspace package cannot reach npm until a reviewed PR adds it to the allowlist, and until then it blocks every release run rather than being skipped. Maintainers may pass explicit package names to select a subset manually; the same whole-set preflight applies.

#### Recovery from publish failure

If the publish step fails after the Version PR is merged (network blip, npm registry error, transient runner issue), first re-run the failed `Release` workflow run from the GitHub Actions UI. Packages are published one at a time. A retry skips any exact version already present on npm, publishes the rest, and emits one exact `New tag: <name>@<version>` line for every version that has no tag yet after the complete set succeeds. This lets `changesets/action` create the missing Git tag and GitHub Release metadata without attempting a duplicate npm publication.

If the Version PR's own push-to-main CI was red, fix forward: the next green push to main publishes what npm lacks and tags what git lacks, with no manual step (re-running CI on the tip has the same effect once it is green). The tag and the provenance attestation then name the commit that published, not the Version PR commit — the honest pointer, since that is the tree the tarballs were packed from.

If no trigger edge is coming (the run is wedged, or its `workflow_run` event was lost), use the same `Release` workflow's **Run workflow** action and provide a full 40-character merged-main commit as `release_sha`, normally the current tip of `main`. The `Recover Publish from Merged Main SHA` job validates that the selected commit is already contained in `main`, requires the CI run for that exact commit to have completed with every required job green, binds the job to the `production` environment, and then runs `pnpm run release-check` followed by the first-publish-protected release chain on a GitHub-hosted runner with OIDC provenance. The SHA chooses the tree that gets packed, not the versions that get published, so pick the newest green commit. The checked-in workflow does not prove a protected `production` environment or required reviewer approval; those rules live in repository settings. That binding puts an `environment: production` claim in the job's OIDC token. npm documents the trusted-publisher environment field only as optional, not whether an empty field admits a job that names one, so configure a second trusted publisher on each package (same owner, repository, and `release.yml`, environment `production`) before relying on this job; npm allows up to ten per package.
...
**If the OIDC exchange fails.** pnpm logs `Skipped OIDC: Failed token exchange request …` and the upload then fails with an authentication error, because no static token exists. On npmjs.com, under each package's Settings → Trusted Publisher, confirm GitHub Actions, owner `b4r7x`, repository `diffgazer`, workflow filename `release.yml` (the filename only, with its extension, case-sensitive) and an environment that matches the job (empty for the release job, `production` for recovery); fix the entry and re-run the failed run. Only as a last resort, when npm will not accept the workflow identity at all: create a temporary granular access token scoped to the four packages with the shortest expiry, add it as the `NPM_TOKEN` repository secret, re-add `NPM_TOKEN: ${{ secrets.NPM_TOKEN }}` to the Recovery job's changesets step in a reviewed PR that also relaxes the release guard, run Recovery, then revoke the token, delete the secret, and revert both changes. pnpm still tries OIDC first and `--provenance` still signs with the job's identity, so the token only authenticates packages whose exchange failed. If "Require two-factor authentication and disallow tokens" is enabled on the packages, relax it for the duration.
...
- `sharp@0.35.4` — allowed. Builds the libvips binding the docs image pipeline needs; the approved
  version tracks the optional `sharp` range `next` declares (`^0.35.4` since `next@16.3.4`).
...
- `js-yaml@4` pinned to `^4.3.2` to clear the high-severity YAML merge-key quadratic-CPU advisories (affects `>=4.0.0 <4.3.0`) and GHSA-2883-xcg3-v3hh (`maxTotalMergeKeys` not bounding CPU for empty merge sources, affects `>=4.0.0 <4.3.2`). Reached transitively through `apps/docs > @tanstack/react-start > @tanstack/start-plugin-core > xmlbuilder2 > js-yaml` and `apps/docs > fumadocs-mdx > js-yaml`. The override is deliberately range-scoped to the v4 line: a blanket `js-yaml` override would force v4 onto the v3 consumers `read-yaml-file` and `@lhci/utils`, which call the v4-removed `safeLoad` API and sit on the release path. Sunset when the v4 consumers resolve a patched version naturally.
