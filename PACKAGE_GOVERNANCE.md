...
The hosted shadcn-style registry at `https://diffgazer.com/r/` is **not yet live**. DNS for `diffgazer.com` does not resolve, so every `npx shadcn add https://diffgazer.com/r/...` snippet in READMEs and docs is gated as future use until publication.
...
1. `host diffgazer.com` resolves to the deploy target.
2. `curl -fI https://diffgazer.com/r/ui/registry.json` returns `200`.
3. `curl -fI https://diffgazer.com/r/ui/button.json` returns `200`.
