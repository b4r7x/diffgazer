...
The hosted shadcn-style registry at `https://docs.diffgazer.b4r7.dev/r/` is **not yet live**. DNS for `docs.diffgazer.b4r7.dev` does not resolve to a serving registry yet, so every `npx shadcn add https://docs.diffgazer.b4r7.dev/r/...` snippet in READMEs and docs is gated as future use until publication.
...
1. `host docs.diffgazer.b4r7.dev` resolves to the deploy target.
2. `curl -fI https://docs.diffgazer.b4r7.dev/r/ui/registry.json` returns `200`.
3. `curl -fI https://docs.diffgazer.b4r7.dev/r/ui/button.json` returns `200`.
