...
const closeReopenExamples = [
  ...inlineExamples,
  {
    name: "empty",
    Example: CommandPaletteEmpty,
    label: "Empty palette",
  },
] as const;
...
describe("CommandPalette gallery examples", () => {
  ...
  it.each(closeReopenExamples)("$name closes from the input suffix and can reopen", async ({
    Example,
    label,
  }) => {
    const user = userEvent.setup();
    ...
    const palette = within(screen.getByRole("group", { name: label }));
    await user.click(palette.getByRole("button", { name: "Close" }));
