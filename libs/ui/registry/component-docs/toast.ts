...
export const toastDoc: ComponentDoc = {
  description: "Terminal-styled toast notification system. Imperative toast() API callable from anywhere. Supports tone shortcuts (severity), four layout variants, promise handling, and position-aware animations.",
  notes: [
    {
    ...
    },
    {
      ...
      content: "`tone` is severity (success/error/warning/info/loading) and drives color and ARIA role. `variant` is the layout shell: `card` (default, full payload), `hud` (single-line pill), `viewfinder` (corner brackets), `countdown` (card + auto-dismiss progress bar).",
    },
    {
    ...
    },
    {
      ...
      content: "Auto-dismiss timers pause while the pointer hovers the toaster region, while focus is inside it (so action buttons stay reachable by keyboard), and while the document tab is hidden. Timers resume from the remaining time once interaction ends — users never lose time they could not see or read.",
    },
  ],
  anatomy: [
  ...
  props: {
    ...
    "toast (function)": {
      ...
      tone: {
        ...
        required: false,
        defaultValue: '"info"',
        description: "Severity tone. Drives icon, color, and auto-dismiss behavior (error and loading persist).",
      },
      variant: {
        type: '"card" | "hud" | "viewfinder" | "countdown"',
        required: false,
        defaultValue: '"card"',
        description: "Layout shell. `card` is the default two-row layout; `hud` is a single-line pill (no body/action); `viewfinder` has corner brackets; `countdown` adds an auto-dismiss progress bar.",
      },
      message: {
        type: "string",
        required: false,
        defaultValue: null,
        description: "Secondary detail text below the title (rendered inline as muted text in `hud`).",
      },
      duration: {
        type: "number",
        required: false,
      ...
      action: {
        ...
        required: false,
        defaultValue: null,
        description: "Action element (e.g. a Button) rendered under the message. Ignored by `hud` (dev warning logged).",
      },
      id: {
        type: "string",
        required: false,
