...
describe("useSelectableCollection", () => {
  ...
  it("degrades to no-op stylesheet observation when the prototype is frozen", async () => {
    const iframe = document.createElement("iframe");
    document.body.append(iframe);
    const frameDocument = iframe.contentDocument;
    const frameWindow = iframe.contentWindow;
    ...
    const framePrototype = getStyleSheetPrototype(frameWindow);
    Object.freeze(framePrototype);
    const frameOriginals = getStyleSheetMethods(framePrototype);
    const queueMicrotask = vi.spyOn(frameWindow, "queueMicrotask");

    const frame = renderCollection({ useSelectableCollection }, frameDocument);
    // Mounting must not throw; the frozen prototype methods stay untouched.
    expectStyleSheetMethods(framePrototype, frameOriginals);

    // Resize/visibilitychange listeners keep scheduling despite the degraded observer.
