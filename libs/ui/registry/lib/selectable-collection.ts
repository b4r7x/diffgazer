...
function subscribeToDocumentChanges(document: Document, subscriber: () => void): () => void {
...
export function useSelectableCollection(containerRef: RefObject<HTMLElement | null>) {
  ...
  useLayoutEffect(() => {
    ...
    const unsubscribeDocument = subscribeToDocumentChanges(container.ownerDocument, syncCollection);
    syncCollection();
