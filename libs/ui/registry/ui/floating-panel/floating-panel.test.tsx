...
import { FloatingPanel, useFloatingPanelContext } from "./index.js"
    ...
    <>
      <button
        ...
        ref={(node) => {
          ...
          mountTriggerRect(node, { x: triggerX, y: triggerY, width: triggerWidth, height: triggerHeight })
        }}
        onClick={() => setOpen((v) => !v)}
      >
        toggle
...
describe("FloatingPanel positioning attributes", () => {
  it.each<[FloatingSide, FloatingAlign, string]>([
    ...
    "reflects resolved side=%s align=%s with positioned flag and transform-origin %s",
    async (side, align, expectedOrigin) => {
      render(<Harness initialOpen side={side} align={align} panelLabel={`origin-${side}-${align}`} />)
...
describe("FloatingPanel a11y", () => {
  ...
  it("has no axe violations with role=dialog and aria-label", async () => {
    const { baseElement } = render(<Harness initialOpen panelRole="dialog" panelLabel="dialog-a11y" />)
...
describe("FloatingPanel exit fallback timer", () => {
  ...
  it("forces unmount once exitFallbackMs elapses without an animationend event", () => {
    ...
    const onExitComplete = vi.fn()
    render(
      <Harness initialOpen panelLabel="fallback" exitFallbackMs={50} onExitComplete={onExitComplete} />,
    )
    ...
    expect(screen.getByRole("dialog", { name: "fallback" })).toHaveAttribute("data-state", "closed")
...
describe("FloatingPanel alignOffset", () => {
  it("shifts the panel cross-axis by alignOffset when align=start on a vertical side", () => {
    render(
      <Harness initialOpen side="bottom" align="start" panelLabel="offset-none" triggerX={100} triggerY={100} />,
    )
