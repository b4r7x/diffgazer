...
import { Menu, type MenuItemProps, type MenuProps } from "./index";
...
describe("Menu", () => {
  ...
  it("marks selected detail items and renders their values", () => {
    ...
    expect(getMenuItem(/delete/i)).toHaveAttribute("data-highlighted");
  });
  ...
  it("renders falsy detail item values that are explicit React nodes", () => {
    render(
      <Menu aria-label="Test menu" variant="detail">
        ...
        <Menu.Item id="none">None</Menu.Item>
      </Menu>,
    );

    expect(screen.getByText("0")).toBeInTheDocument();
    // An empty string still renders its own value cell; an omitted value renders none.
    expect(getMenuItem("Empty").children.length).toBe(getMenuItem("None").children.length + 1);
  });
