...
- **NavigationList**: `selectedId` / `onSelect`, with optional `highlightedId` / `onHighlightChange` for controlled keyboard highlight
...
| Menu | `Menu` | `MenuItem`, `MenuDivider` | `selectedId`, `highlightedId`, `onHighlightChange`, `onSelect`, `variant` |
| NavigationList | `NavigationList` | `NavigationListItem`, `NavigationListItemTitle`, `NavigationListItemStatus`, `NavigationListItemMeta`, `NavigationListBadge`, `NavigationListSubtitle` | `selectedId`, `highlightedId`, `onHighlightChange`, `onSelect`, `onEnter`, `onNavigationBoundaryReached`, `autoFocus`, `focused` |
| Toast | `Toaster` | none | Shared toast store through `toast()` helpers |
