...
| `NavigationList` | `onKeyDown`, `highlightedId`, `onHighlightChange`, `focused`, `selectedId`, `onSelect` |
...
- **`selectedId`** -- the currently selected item; for `CommandPalette`, use `highlightedId` because `selectedId` is only a deprecated highlight-state alias
