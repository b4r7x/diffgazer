...
| `stepper-variants.ts` | Stepper |
| `horizontal-stepper-variants.ts` | HorizontalStepper |
...
The control simply becomes 44px on coarse pointers. This is the default recipe and by far the most used one: `CodeBlock`'s copy button (via `code-block.css`, which also grows the header row so the taller control is not clipped), `Toast`'s close button, `SearchInput`'s clear button, `Sidebar.Trigger`, sidebar rows and section titles, the `Checkbox`/`Radio` row (`selectable-variants.ts`), and `Tabs`/`ToggleGroup` at `sm` (`segmented-variants.ts`). Grep `pointer-coarse:min-h-11` for the live list.
