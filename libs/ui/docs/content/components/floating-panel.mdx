...
Emits a one-time `console.warn` per panel element when neither `role` nor `aria-label`/`aria-labelledby` is supplied.
...
The panel reads these CSS custom properties from the `.ui-floating-panel` cascade. Override on the panel, an ancestor, or `:root`. The animation tokens collapse to fade under `prefers-reduced-motion: reduce` (see [Theme overview](../theme/overview#floating-surface-animation-tokens)).
