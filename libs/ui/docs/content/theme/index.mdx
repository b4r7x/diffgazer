...
`--surface-1` is one step above `--base-bg` in the dark theme and the elevated card surface in the light theme. Override it on `:root` (or any ancestor) to tint every elevated panel without forking the component:
