...
> **Warning:** `@diffgazer/add`, `@diffgazer/ui`, and `@diffgazer/keys` are publish-gated until the npm `view` checks in PACKAGE_GOVERNANCE.md pass. The package-manager commands below work only after `npm view @diffgazer/add version`, `npm view @diffgazer/ui version`, and `npm view @diffgazer/keys version` succeed. Until then, use the [pre-publication local validation](#pre-publication-local-validation) flow.
...
Verify availability first with `npm view @diffgazer/add version`. If it is not published yet, use the pre-publication flow below.
