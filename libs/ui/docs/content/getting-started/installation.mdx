...
The one-shot `pnpm dlx`/`npx @diffgazer/add` commands work only if `@diffgazer/add` is published — see [Package managers](#package-managers).
...
`@diffgazer/add` is publish-gated and not on npm, so these one-shot commands fail today. They work only if the package is published:
