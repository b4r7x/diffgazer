...
`@diffgazer/ui` has two intended consumption modes. The npm packages are external publish-gated as of May 6, 2026, so public npm commands stay gated until `npm view @diffgazer/add version`, `npm view @diffgazer/ui version`, and `npm view @diffgazer/keys version` succeed.
