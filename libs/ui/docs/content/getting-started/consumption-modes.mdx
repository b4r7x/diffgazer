...
`@diffgazer/ui` has two intended consumption modes. The npm packages are publish-gated until the npm `view` checks in PACKAGE_GOVERNANCE.md pass, so public npm commands stay gated until `npm view @diffgazer/add version`, `npm view @diffgazer/ui version`, and `npm view @diffgazer/keys version` succeed.
