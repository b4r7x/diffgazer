...
> **npm packages:** `@diffgazer/add`, `@diffgazer/ui`, and `@diffgazer/keys` are publish-gated and not on npm. Copy-first mode works without a checkout through the hosted registry (`npx shadcn add https://r.b4r7.dev/r/ui/<item>.json`) or through `dgadd` from a packed tarball; runtime package mode needs tarballs packed from a checkout — see the canonical [Consumption Paths](https://github.com/b4r7x/diffgazer#consumption-paths) section.

## Copy-first mode

Use copy-first mode when the consuming app should own and customize component source. The packed `@diffgazer/add` tarball installed in the target app is what puts `dgadd` on `pnpm exec` ([Copy-first mode procedure](https://github.com/b4r7x/diffgazer#copy-first-mode-dgadd)).
...
Install both packages when using this path. They are not on npm, so pack them from a checkout of the repository and install the tarballs:
...
If the packages are published, this becomes:
