...
> **Before publication:** Diffgazer packages are not yet published to npm, so neither mode installs from the public registry yet. Pack the workspace packages from a checkout and install those tarballs in the target app — see the canonical [Consumption Paths](https://github.com/b4r7x/diffgazer#consumption-paths) section.

## Copy-first mode

Use copy-first mode when the consuming app should own and customize component source. Before publication, the packed `@diffgazer/add` tarball installed in the target app is what puts `dgadd` on `pnpm exec` ([Copy-first mode procedure](https://github.com/b4r7x/diffgazer#copy-first-mode-dgadd)).
