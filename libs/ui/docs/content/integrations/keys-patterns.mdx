...
@diffgazer/ui components are visual-only -- they don't handle keyboard events internally. @diffgazer/keys provides the keyboard layer. Here are the common patterns for integrating the two.
...
```tsx
import { useRef } from "react"
import { useScope, useFocusTrap, useScrollLock, useKey } from "@diffgazer/keys"
import {
