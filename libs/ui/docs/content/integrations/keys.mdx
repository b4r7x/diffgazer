...
> **Note:** `@diffgazer/ui` and `@diffgazer/keys` are publish-gated and not on npm; pack them from the repository and install those tarballs.
...
Install the package. It is not on npm, so pack it from a checkout of the repository and install that tarball ([Consumption Paths](https://github.com/b4r7x/diffgazer#consumption-paths)):
...
If `@diffgazer/keys` is published, this becomes:
