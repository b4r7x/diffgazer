...
`Dialog` uses a CSS-only `body:has(dialog[open])` scroll lock with `scrollbar-gutter: stable` compensation. Use `useScrollLock` as the standalone alternative for app-owned overlays or custom scroll containers.
