...
describe("UserConfigSchema refine — model/provider validation", () => {
  it.each<{
    ...
    {
      name: "zai with a fresh GLM id",
      overrides: { provider: "zai", model: "glm-5.1" },
      success: true,
    },
    {
      name: "zai-coding with a GLM id",
      overrides: { provider: "zai-coding", model: "glm-4.7" },
      success: true,
    },
    {
      name: "openrouter with arbitrary model id",
      overrides: { provider: "openrouter", model: "any-model-id/whatever" },
      success: true,
    },
    {
      name: "groq with a fresh id",
      overrides: { provider: "groq", model: "meta-llama/llama-4-scout-17b-16e-instruct" },
      success: true,
    },
    {
      name: "cerebras with a fresh id",
    ...
    {
      name: "any provider with a whitespace-only model string",
      overrides: { model: "   " },
      success: false,
    },
