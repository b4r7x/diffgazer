---
title: Scopes
description: Isolate keyboard handlers with the `@diffgazer/keys` scope stack.
---

> Try the [Scoped Dialog demo](https://github.com/b4r7x/diffgazer/tree/main/libs/keys/examples/playground) to see scopes in action.
...
The scope with the highest React `useId` order-segments wins. Precedence is a depth-major comparison of the parsed `useId` of each `useScope` call: segments are compared left to right, and the larger segment at the first difference takes priority. On the client, React assigns those ids in registration order, so this resolves to "the most recently mounted declarative scope wins" — a nested confirmation dialog that mounts in the same commit as its parent modal becomes active because it mounts after it. Imperative `pushScope` calls outrank all declarative scopes until popped.

> Under SSR/hydration, React bit-packs each id's tree position into a single value, so a more deeply nested scope in an earlier branch can produce a larger segment and outrank a shallower scope in a later branch — i.e. on the server the same comparison can resolve to deepest-wins. Keep mutually exclusive scopes (e.g. modal vs. its confirm dialog) in a parent→child nesting relationship rather than relying on sibling-branch ordering.
...
function Panel({ isolate }: { isolate: boolean }) {
  // No scope until isolate flips to true
