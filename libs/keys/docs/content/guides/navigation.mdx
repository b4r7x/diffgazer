---
title: Navigation
description: Build list, menu, and tab navigation with `@diffgazer/keys`.
---

> Try the [List Navigation](https://github.com/b4r7x/diffgazer/tree/main/libs/keys/examples/playground) and [Tab Bar](https://github.com/b4r7x/diffgazer/tree/main/libs/keys/examples/playground) demos to see navigation in action.
...
2. A keyboard entry point, such as `tabIndex={0}` on an `aria-activedescendant` listbox
3. Child elements with a matching **`role`** attribute (`"option"`, `"menuitem"`, `"menuitemcheckbox"`, `"menuitemradio"`, `"radio"`, `"checkbox"`, `"button"`, or `"tab"`)
4. Each child has a **`data-value`** attribute -- this is how the hook tracks which item is highlighted
...
function FileList() {
  ...
  const { highlighted, isHighlighted, onKeyDown } = useNavigation<string>({
    containerRef,
    role: "option",
    defaultHighlighted: "source",
  ...
  return (
    <div
      ...
      aria-activedescendant={highlighted ? `file-${highlighted}` : undefined}
      tabIndex={0}
      onKeyDown={onKeyDown}
    >
      {files.map((file) => (
        <div
          ...
          role="option"
          data-value={file.id}
          aria-disabled={file.disabled ? "true" : undefined}
          aria-selected={isHighlighted(file.id)}
          style={{ background: isHighlighted(file.id) ? "#e0e7ff" : undefined }}
        >
          {file.label}
        </div>
      ))}
...
interface UseNavigationOptions {
  ...
  itemSelector?: string;            // narrow items when the role query is broader
