...
| `ArrowUp` | actions | Exits the actions zone back to content when `canExitActions` is `true`, moving focus to `disabledFocusFallbackRef`. Without that ref it only blurs the action, so focus lands on `document.body`. |
