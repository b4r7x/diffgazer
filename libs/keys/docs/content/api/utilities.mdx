---
title: Utilities
description: Helper functions for working with `@diffgazer/keys` — keys, navigation item utilities, composed-tree and reachability predicates, DOM focus restore utilities, and keyboard context hooks.
---
...
Handlers created through `keys()` follow the same [KeyHandler](/keys/api/types) contract as any other handler: returning `false` declines the match and lets the next lower-priority handler in the active scope run.
