...
Hotkeys are strings like `"mod+k"`, `"shift+Enter"`, or `"ArrowDown"`. Each string is parsed once when the handler registers, then matched against keyboard events at dispatch time, and used with [useKey](/keys/hooks/use-key).
