  ...
  it("accepts valid drilldown result with all fields", () => {
    const result = DrilldownResultSchema.safeParse({
      ...
      issue: {
        ...
        whyItMatters: "Could cause runtime crash",
        evidence: [
          {
            type: "code",
            title: "Code at src/app.ts:10",
            sourceId: "src/app.ts:10-15",
            file: "src/app.ts",
  ...
  it("accepts drilldown with empty related issues and references", () => {
    const result = DrilldownResultSchema.safeParse({
      ...
      issue: {
        ...
        suggested_patch: null,
        confidence: 0.7,
        symptom: "Inefficient algorithm",
        whyItMatters: "Impacts response time",
        evidence: [],
      },
      detailedAnalysis: "Test",
      rootCause: "Test",
