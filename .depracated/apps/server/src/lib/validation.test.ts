...
describe("parseAndValidate", () => {
  ...
  type ErrorCode = "PARSE_ERROR" | "VALIDATION_ERROR";

  const parseErrorFactory = (message: string): { code: ErrorCode; message: string } => ({
    code: "PARSE_ERROR",
    message,
  });

  const validationErrorFactory = (message: string): { code: ErrorCode; message: string } => ({
