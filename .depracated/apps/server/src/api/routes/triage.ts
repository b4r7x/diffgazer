...
const triage = new Hono<ProjectEnv>();
...
triage.get("/stream", async (c) => {
  ...
  console.log("[ROUTE:STREAM] Params:", { mode, lenses, files, profile, projectPath });

  return streamSSE(c, async (stream) => {
    console.log("[ROUTE:SSE] Stream started");
    try {
      await streamTriageToSSE(clientResult.value, { mode, files, lenses, profile, projectPath }, stream);
...
triage.post("/reviews/:id/drilldown", async (c) => {
  ...
  const drilldownProjectPath = getProjectPath(c);
  const gitService = createGitService({ cwd: drilldownProjectPath });
