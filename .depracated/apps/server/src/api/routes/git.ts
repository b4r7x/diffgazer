...
async function getGitService(c: Context<ProjectEnv>, path: string | undefined): Promise<GetGitServiceResult> {
  if (path && !isRelativePath(path)) {
    return { ok: false, response: errorResponse(c, "Invalid path", ErrorCode.INVALID_PATH, 400) };
  }

  const basePath = getProjectPath(c);
