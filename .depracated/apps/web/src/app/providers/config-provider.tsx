...
export function ConfigProvider({ children }: { children: ReactNode }) {
    ...
    const applyConfigData = useCallback((data: ConfigData) => {
        ...
        setIsConfigured(data.providers.some(p => p.isActive));
    ...
    const updateAfterAction = useCallback(async (
        newProvider: AIProvider | undefined,
        newModel: string | undefined
    ) => {
        const providers = await getProviderStatus();
        const { projectId, repoRoot, trust } = stateRef.current;
        const data: ConfigData = {
    ...
    const deleteProviderCredentials = useCallback(async (providerName: AIProvider) => {
        ...
        try {
            ...
            await updateAfterAction(wasActive ? undefined : provider, wasActive ? undefined : model);
        } catch (err) {
