...
import { createStorageTestContext, createPathsMock, unwrap } from "../../__test__/testing.js";

const mocks = vi.hoisted(() => ({ testDir: "" }));

vi.mock("../storage/paths.js", async () => ({
  get paths() {
...
describe("Vault Security", () => {
  ...
  beforeEach(async () => {
    const context = await createStorageTestContext("vault");
    mocks.testDir = context.testDir;
    cleanup = context.cleanup;

    secretsDir = join(context.testDir, "secrets");
