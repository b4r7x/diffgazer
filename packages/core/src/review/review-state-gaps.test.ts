...
describe("reviewReducer — gap coverage", () => {
  ...
  describe("updateAgents — agent_progress", () => {
    it("updates progress and message", () => {
      ...
      state = reviewReducer(state, {
        ...
        event: {
          type: "agent_start",
          agent: { id: "optimizer", lens: "performance", name: "Optimizer", badgeLabel: "PERF", badgeVariant: "info", description: "Perf" },
          timestamp: ts,
        },
      });
  ...
  describe("file progress", () => {
    ...
    it("adds file to completed set on file_complete", () => {
      ...
      expect(state.fileProgress.completed.has("src/app.ts")).toBe(true);
      expect(state.fileProgress.currentFile).toBeNull();
    });
    ...
    it("ignores file_complete with agent scope", () => {
      ...
      expect(state.fileProgress.completed.has("src/app.ts")).toBe(false);
    });
  });
  ...
  describe("tool_call with readFileContext — colon parsing", () => {
    ...
    it("uses full input when no colon present", () => {
      ...
      expect(state.fileProgress.completed.has("src/index.ts")).toBe(true);
    });
  });
