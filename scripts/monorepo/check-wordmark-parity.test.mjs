...
const WEB_HEADER = "apps/web/src/components/layout/header.tsx";
...
test("the docs wordmark is generated from the art the web header hardcodes", () => {
  assert.equal(
    extractDocsConstant("WORDMARK_ASCII"),
    extractWebRows().join("\n"),
    `${DOCS_GENERATED} and ${WEB_HEADER} disagree. Both render the same brand mark; re-sync the figlet font, casing, and trailing-blank-row trim.`,
  );
});
...
test("the landing hero inlines the same art, row for row", () => {
  assert.deepEqual(
    ...
    extractWebRows(),
    `${LANDING_PAGE} and ${WEB_HEADER} disagree. The hero art is inlined by hand; copy the rows across including their trailing padding.`,
  );
});
