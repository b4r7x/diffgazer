...
test("copied logo-figlet enters the Vite graph through both static lazy font loaders", () => {
  const app = buildSmokeApp(
    [],
    ["@/components/ui/logo/figlet", "@/components/ui/logo/figlet-text"],
  );

  assert.match(app, /import \{ getFigletText \} from '@\/components\/ui\/logo\/figlet';/);
  assert.match(app, /getFigletText\('DG', 'Big'\)/);
  assert.match(app, /getFigletText\('DG', 'Small'\)/);
  assert.doesNotMatch(app, /import '@\/components\/ui\/logo\/figlet(?:-text)?';/);
});
