...
import { parseCliCompatibilityRecord } from "./cli-compatibility.js";
import { parseCopilotJsonlTerminal } from "./copilot-cli.js";
...
describe("Copilot JSONL terminal contract", () => {
  ...
  it("is the single parser used by both the compatibility probe and the runtime adapter", async () => {
    ...
    const [probeSource, runtimeSource] = await Promise.all([
      ...
      readFile(path.join(directory, "copilot-cli.ts"), "utf8"),
    ]);

    for (const source of [probeSource, runtimeSource]) {
      expect(source).toContain('from "./copilot-jsonl.js"');
