...
let testDependencies: LocalHttpDependencies = {};

/** Test-only hook for injecting fetch/DNS without changing the public adapter contract. */
export function setLocalHttpTestDependencies(dependencies: LocalHttpDependencies): void {
  testDependencies = dependencies;
}

export function localHttpTestDependencies(): LocalHttpDependencies {
...
export function resolveLocalHttpDependencies(
  ...
  return {
    fetch: dependencies.fetch ?? testDependencies.fetch ?? globalThis.fetch,
    lookup: dependencies.lookup ?? testDependencies.lookup,
    now: dependencies.now ?? testDependencies.now ?? defaultNow,
  };
}
