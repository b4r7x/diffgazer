...
const MAX_FORCE_KILL_DELAY_MS = 2_147_482_647;

function readForceKillDelay(value: string | undefined, fallback: number): number {
  if (value === undefined) return fallback;
  const parsed = Number(value.trim());
  return Number.isInteger(parsed) && parsed > 0 && parsed <= MAX_FORCE_KILL_DELAY_MS
    ? parsed
    : fallback;
}
...
const forceKillMs = readForceKillDelay(process.env.DIFFGAZER_FORCE_KILL_DELAY_MS, 2000);
