...
import { focusBorder, rowTone, selectionFill } from "./chrome";
...
describe("selection fill", () => {
  test.each(PALETTES)("%s selects in the blue family, never in accent", (_name, tokens) => {
    expect(selectionFill(tokens)).toBe(tokens.blue);
    expect(selectionFill(tokens)).not.toBe(tokens.accent);
  });

  test.each(PALETTES)("%s keeps row text on the fill above 4.5:1", (_name, tokens) => {
    const tone = rowTone(tokens, { isHighlighted: true });
    expect(contrast(tone.primary, selectionFill(tokens))).toBeGreaterThanOrEqual(4.5);
    expect(contrast(tone.secondary, selectionFill(tokens))).toBeGreaterThanOrEqual(4.5);
  });
});
...
describe("focusBorder", () => {
  ...
  test.each(PALETTES)("%s borders a focused pane in the row hue too", (_name, tokens) => {
    ...
    expect(focusBorder(tokens, true)).toBe(selectionFill(tokens));
  });
});
...
describe("rowTone", () => {
  test("paints the highlighted row of a focused list", () => {
    ...
    expect(tone.background).toBe(selectionFill(darkPalette));
  ...
  test("names the highlighted row without painting it while the list is inactive", () => {
    ...
    expect(tone.primary).toBe(selectionFill(darkPalette));
