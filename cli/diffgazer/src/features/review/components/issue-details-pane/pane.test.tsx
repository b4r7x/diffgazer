...
describe("IssueDetailsPane (TUI) evidence presentation", () => {
  ...
  test("renders the Better Options heading with each suggested alternative", () => {
    const issue = makeIssue({
      betterOptions: [
    ...
    expect(frame).toContain("BETTER OPTIONS");
    expect(frame).toContain("Use a parameterized query instead of string concatenation");
    expect(frame).toContain("Validate the redirect target against an allowlist");
  });
});
