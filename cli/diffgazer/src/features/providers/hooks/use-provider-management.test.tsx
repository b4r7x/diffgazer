...
import { buildProviderRows, GEMINI_CONFIGURATION } from "@diffgazer/core/testing/provider-fixtures";
...
describe("useProviderManagement single flight", () => {
  it("dispatches one create for two activations in the same frame", async () => {
    ...
    await act(async () => {
      void hook.result.current.handleCreateConfiguration(owner, GEMINI_INPUT, { acknowledgement });
      void hook.result.current.handleCreateConfiguration(owner, GEMINI_INPUT, { acknowledgement });
    });
