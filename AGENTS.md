...
- `apps/landing` owns the marketing landing page. It is a vanilla HTML/TS Vite surface (no React, no Tailwind): it consumes `@diffgazer/ui/styles.css` (design tokens plus the pure-CSS Panel/DiffView data-attribute contracts the hero composes; the only workspace dependency, enforced by `src/boundary.test.ts`), self-hosts the Departure Mono display face (OFL, `public/fonts/`), and carries no product/domain logic and no docs utilities. All motion is hand-written TS under `src/effects/` gated behind `prefers-reduced-motion`. Docs/GitHub link targets come from `VITE_DOCS_ORIGIN` / `VITE_GITHUB_URL` build envs with hardcoded fallbacks. Revisit the no-framework choice only if landing grows into a multi-page content site.
- `libs/keys/artifacts` (`@diffgazer/keys-artifacts`) is a private, never-published parity mirror of `libs/keys/dist/artifacts`, with no build script, no dependencies, and no dependents. Its `artifacts/**` payload is build output written by the `@diffgazer/keys` build, regenerated by `pnpm run prepare:artifacts` and parity-checked against the source tree by `scripts/monorepo/validate-artifacts.mjs`.
- `libs/keys/examples/playground` (`@diffgazer/keys-playground`) is a private dev-only Vite host for exercising `libs/keys` hooks by hand, started through the root `keys:playground` script. It ships nothing and is not a documentation surface — anything worth documenting belongs in `apps/docs`.
...
- **UI surfaces (`apps/web`, `apps/docs`, `apps/landing`, `cli/diffgazer` Ink TUI).** Bulletproof-react taxonomy: `app/` for providers, routers, and thin routes where the surface owns that shell; on-demand `features/<x>/{components,hooks,lib}`; shared `components/{ui,shared,layout}`, `hooks/`, `lib/`, `config/`, `types/`, and `testing/`. `apps/docs` keeps the TanStack-owned `routes/` root plus `router.tsx`/`client.tsx`/`server.ts`; `apps/landing` is the exception — a vanilla HTML/TS page (flat `src/` + `src/effects/`, no React taxonomy) kept intentionally small; `apps/web` keeps `utils/`; `cli/diffgazer` keeps `lib/servers/` launcher adapters and `theme/`, plus feature-local test helpers under `features/<x>/testing/` (as in `features/history/testing/`) when a fixture serves only that feature. Flat-sibling colocation; no internal barrels.
...
- **Library packages (`libs/core`, `libs/ui`, `libs/keys`, `libs/registry`).** Organized by domain module; no `features/` taxonomy. `libs/core` is subpath-only and deliberately has no root `.` entry; every public entry is an explicit named subpath in its package manifest. `libs/keys` and `libs/registry` use one `src/index.ts` root entry plus the package subpaths declared in their manifests. `libs/ui` source lives under `registry/` in per-component, hook, and lib folders; it ships package subpaths plus copy/shadcn registry items and has no `src/index.ts`.
...
- Registry examples must stay pasteable into an RSC app: an example that hands a function to a component — a render-prop child or a JSX event handler — declares `"use client"` as its first statement, because a copied module is a server module until it says otherwise. Enforced by `validate-registry-metadata`; do not weaken the gate.
...
- `dgadd remove` must respect ownership metadata and must not remove copied shared dependencies still needed by retained installed items. Install-time dependency edges are persisted on each manifest entry (`requires`) so cascade removal uses the graph from install, not the live registry bundle, after registry dependency drift. Manifests without persisted `requires` edges do not auto-cascade transitive removals; blocking still consults the live registry for retained owners that lack persisted edges.
...
- Shipped registry CSS is tested only at the documented-contract level (jsdom cannot compute CSS): assert presence of documented data-attribute selectors/declarations, or a guard for a commented concrete regression, through the shared `registry/testing/css-contract` reader. Never regex-count occurrences, snapshot rule shape, or match whitespace-exact strings. Real layout behavior belongs to browser-based tests, not text assertions.
