...
- `libs/keys/artifacts` (`@diffgazer/keys-artifacts`) is a private, never-published mirror workspace that packages the built `libs/keys/dist/artifacts` tree so the docs artifact handoff is exercised as a real package. Its `artifacts/**` payload is build output, regenerated by `pnpm run prepare:artifacts` and parity-checked against the source tree by `scripts/monorepo/validate-artifacts.mjs`.
